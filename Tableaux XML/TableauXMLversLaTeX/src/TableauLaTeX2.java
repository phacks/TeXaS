...
public class TableauLaTeX2 {
			...
			if (indiceLigne+1<nbLignes){
				...
				if (lesRupturesDeBordures.length>0){
					for (int j = 0; j < lesRupturesDeBordures.length-1; j++) {
						codeLaTeXTableau = codeLaTeXTableau + " \\cline{"+(lesRupturesDeBordures[j]+2)+"-"+(lesRupturesDeBordures[j+1])+"}";
					}
					codeLaTeXTableau = codeLaTeXTableau + " \\cline{" + (lesRupturesDeBordures[lesRupturesDeBordures.length-1]+2) + "-" + nbColonnes +"} \n";
				}
				else{
	...
	// Méthode renvoyant les indices des false dans une ligne, sous forme de tableau d'indice
	private int[] lesElementFalse(boolean[] tableauLigne){
		...
		for (int i = 0; i < resultat.length; i++) {
			if ((i==0 && tableauLigne[i])){
				resultat[i] = i;
			}		
			else if (i>0 && !tableauLigne[i] && tableauLigne[i-1]){
				resultat[i]=i;
			}
		}
		return resultat;
